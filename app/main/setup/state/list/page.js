...
const page = () => {
  const uiState = 'setup.state.state-listing';
  const title = 'State';
  ...
  const onSearch = async (e) => {
    ...
    try {
      const response = await StateService.search(uiData.page, pageSize, uiData.sort, uiData.sortDir, e);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const goto = (path) => {
    saveUIState();
    router.push(`/main/setup/state/${path}`);
  }
  ...
  const onConfirmDelete = async () => {
    const b = await StateService.remove(deleteModal.id);
    if (b) {
