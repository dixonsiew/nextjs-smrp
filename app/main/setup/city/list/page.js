...
const page = () => {
  ...
  const load = async () => {
    ...
    try {
      const response = await CityService.list(listStore.getPage(), pageSize, listStore.getSort(), listStore.getSortDir());
      setUiData(prev => ({
        ...prev,
        init: false,
        loading: false,
  ...
  const onSearch = async (e) => {
    ...
    setUiData(prev => ({ ...prev, loading: true }));
    try {
      const response = await CityService.search(listStore.getPage(), pageSize, listStore.getSort(), listStore.getSortDir(), e);
      setUiData(prev => ({
        ...prev,
        init: false,
        loading: false,
  ...
  const goto = (path) => {
    listStore.setScroll(window.scrollX, window.scrollY);
    router.push(`/main/setup/city/${path}`);
  }
