...
const page = () => {
  const uiState = 'setup.country.country-listing';
  const title = 'Country';
  ...
  const load = async () => {
    ...
    try {
      const response = await CountryService.list(uiData.page, pageSize, uiData.sort, uiData.sortDir);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const onSearch = async (e) => {
    ...
    try {
      const response = await CountryService.search(uiData.page, pageSize, uiData.sort, uiData.sortDir, e);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const goto = (path) => {
    saveUIState();
    router.push(`/main/setup/country/${path}`);
  }
  ...
  const onConfirmDelete = async () => {
    const b = await CountryService.remove(deleteModal.id);
    if (b) {
