...
const page = () => {
  const uiState = 'setup.person-category-code.person-category-code-listing';
  ...
  const load = async () => {
    ...
    try {
      const response = await PersonCategoryCodeService.list(uiData.page, pageSize, uiData.sort, uiData.sortDir);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const onSearch = async (e) => {
    ...
    try {
      const response = await PersonCategoryCodeService.search(uiData.page, pageSize, uiData.sort, uiData.sortDir, e);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const goto = (path) => {
    saveUIState();
    router.push(`/main/setup/person-category-code/${path}`);
  }
  ...
  const onConfirmDelete = async () => {
    const b = await PersonCategoryCodeService.remove(deleteModal.id);
    if (b) {
