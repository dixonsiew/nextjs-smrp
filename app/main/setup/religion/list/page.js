...
const page = () => {
  const uiState = 'setup.religion.religion-listing';
  ...
  const load = async () => {
    ...
    try {
      const response = await ReligionService.list(uiData.page, pageSize, uiData.sort, uiData.sortDir);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const onSearch = async (e) => {
    ...
    try {
      const response = await ReligionService.search(uiData.page, pageSize, uiData.sort, uiData.sortDir, e);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const goto = (path) => {
    saveUIState();
    router.push(`/main/setup/religion/${path}`);
  }
  ...
  const onConfirmDelete = async () => {
    const b = await ReligionService.remove(deleteModal.id);
    if (b) {
