...
import DeliveryTypeService from '../../../../api/setup/delivery-type';
import AppConstant from '../../../../constants';
import { useListStore } from './listStore';

const page = () => {
  const title = 'Birth Delivery Type';
  ...
  const load = async () => {
    ...
    try {
      const response = await DeliveryTypeService.list(listStore.getPage(), pageSize, listStore.getSort(), listStore.getSortDir());
      setUiData(prev => ({
        ...prev,
        init: false,
        loading: false,
  ...
  const onSearch = async (e) => {
    ...
    setUiData(prev => ({ ...prev, loading: true }));
    try {
      const response = await DeliveryTypeService.search(listStore.getPage(), pageSize, listStore.getSort(), listStore.getSortDir(), e);
      setUiData(prev => ({
        ...prev,
        init: false,
        loading: false,
  ...
  const goto = (path) => {
    listStore.setScroll(window.scrollX, window.scrollY);
    router.push(`/main/setup/delivery-type/${path}`);
  }
  ...
  const onConfirmDelete = async () => {
    const b = await DeliveryTypeService.remove(deleteModal.id);
    if (b) {
