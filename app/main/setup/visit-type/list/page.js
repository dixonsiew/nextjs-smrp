...
import VisitTypeService from '../../../../api/setup/visit-type';
import AppConstant from '../../../../constants';

const page = () => {
  const uiState = 'setup.visit-type.visit-type-listing';
  ...
  const load = async () => {
    ...
    try {
      const response = await VisitTypeService.list(uiData.page, pageSize, uiData.sort, uiData.sortDir);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const onSearch = async (e) => {
    ...
    try {
      const response = await VisitTypeService.search(uiData.page, pageSize, uiData.sort, uiData.sortDir, e);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const goto = (path) => {
    saveUIState();
    router.push(`/main/setup/visit-type/${path}`);
  }
  ...
  const onConfirmDelete = async () => {
    const b = await VisitTypeService.remove(deleteModal.id);
    if (b) {
