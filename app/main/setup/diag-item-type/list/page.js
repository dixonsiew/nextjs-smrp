...
const page = () => {
  const uiState = 'setup.diag-item-type.diag-item-type-listing';
  ...
  const load = async () => {
    ...
    try {
      const response = await DiagItemTypeService.list(uiData.page, pageSize, uiData.sort, uiData.sortDir);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const onSearch = async (e) => {
    ...
    try {
      const response = await DiagItemTypeService.search(uiData.page, pageSize, uiData.sort, uiData.sortDir, e);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const goto = (path) => {
    saveUIState();
    router.push(`/main/setup/diag-item-type/${path}`);
  }
  ...
  const onConfirmDelete = async () => {
    const b = await DiagItemTypeService.remove(deleteModal.id);
    if (b) {
