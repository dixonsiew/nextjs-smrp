...
const page = () => {
  const uiState = 'setup.adm-status.adm-status-listing';
  ...
  const load = async () => {
    ...
    try {
      const response = await AdmStatusService.list(uiData.page, pageSize, uiData.sort, uiData.sortDir);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const onSearch = async (e) => {
    ...
    try {
      const response = await AdmStatusService.search(uiData.page, pageSize, uiData.sort, uiData.sortDir, e);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const goto = (path) => {
    saveUIState();
    router.push(`/main/setup/adm-status/${path}`);
  }
  ...
  const onConfirmDelete = async () => {
    const b = await AdmStatusService.remove(deleteModal.id);
    if (b) {
