...
const page = () => {
  const uiState = 'setup.ward-class.ward-class-listing';
  ...
  const load = async () => {
    ...
    try {
      const response = await WardClassService.list(uiData.page, pageSize, uiData.sort, uiData.sortDir);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const onSearch = async (e) => {
    ...
    try {
      const response = await WardClassService.search(uiData.page, pageSize, uiData.sort, uiData.sortDir, e);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const goto = (path) => {
    saveUIState();
    router.push(`/main/setup/ward-class/${path}`);
  }
  ...
  const onConfirmDelete = async () => {
    const b = await WardClassService.remove(deleteModal.id);
    if (b) {
