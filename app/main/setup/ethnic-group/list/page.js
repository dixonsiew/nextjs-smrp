...
const page = () => {
  const uiState = 'setup.ethnic-group.ethnic-group-listing';
  ...
  const load = async () => {
    ...
    try {
      const response = await EthnicGroupService.list(uiData.page, pageSize, uiData.sort, uiData.sortDir);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const onSearch = async (e) => {
    ...
    try {
      const response = await EthnicGroupService.search(uiData.page, pageSize, uiData.sort, uiData.sortDir, e);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const goto = (path) => {
    saveUIState();
    router.push(`/main/setup/ethnic-group/${path}`);
  }
  ...
  const onConfirmDelete = async () => {
    const b = await EthnicGroupService.remove(deleteModal.id);
    if (b) {
