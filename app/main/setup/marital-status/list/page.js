...
const page = () => {
  const uiState = 'setup.marital-status.marital-status-listing';
  ...
  const load = async () => {
    ...
    try {
      const response = await MaritalStatusService.list(uiData.page, pageSize, uiData.sort, uiData.sortDir);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const onSearch = async (e) => {
    ...
    try {
      const response = await MaritalStatusService.search(uiData.page, pageSize, uiData.sort, uiData.sortDir, e);
      setUiData(prev => ({
        ...prev,
        loading: false,
        list: response.data,
  ...
  const goto = (path) => {
    saveUIState();
    router.push(`/main/setup/marital-status/${path}`);
  }
  ...
  const onConfirmDelete = async () => {
    const b = await MaritalStatusService.remove(deleteModal.id);
    if (b) {
